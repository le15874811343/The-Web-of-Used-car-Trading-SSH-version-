            ...
            <p>
				...
				<div class="mr-detail">
					<div class="mr-tab clearfix">
						<a class="on" href="MemberCenter_showmenberOrder.action">我的订单</a>
						<a href="MemberCenter_showsrdz.action">私人订制</a>
						<a href="MemberCenter_showmenberMc.action">出售中的车</a>
					...
					<div class="me-box">
					 <div class="me-dl"  style="display:block;">
							...
							</p><div class="price clearfix" style="margin:5px 0;"><div class="left dInline pNum">	<font>一口价</font><br/><span class="num nBlue">${szCar.value.CPrice}</span><font> 万元</font>	状态：${sta }		 </div>			<span class="num1 left" style="height:43px;padding-top:10px;"><em>新车价：${szCar.value.newprice }万元</em><br/>				为您节省：${szCar.value.newprice-szCar.value.CPrice }万元 							</span>		</div> 	<div class="cs_bt clearfix">			<a  href="MemberCenter_qxdd.action?uid=${szCar.value.UId}&cid=${szCar.value.CId}" class="cs-q">取消订单</a>	
										<a href="MemberCenter_qrsh.action?cid=${szCar.value.CId}&uid=${szCar.value.UId}">确认收货</a>		</div>	</div></li></ul><div class="hPages">    </div>
							</c:forEach>
							</c:if>
								</div>
								 <div class="pages"> <c:if test="${curPage > 1}">
					<a href="MemberCenter_showmenberOrder.action?jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="MemberCenter_showmenberOrder.action?jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="MemberCenter_showmenberOrder.action?jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="MemberCenter_showmenberOrder.action?jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
