...
function sendLogin(){
 ...
 var url = "<%=basePath%>UserInfo_needLogin.action?op=needLogin&nocahe="+new Date().getTime()+"";
  ...
  <a class="bottom_a next_a" style="opacity: 0.7; "></a> 
    ...
    <div class="buyTxt right dInline">
      ...
      <div class="byDl clearfix">
        ...
        <c:if test="${!empty detailsMap.value.newprice }">
        <div class="right dInline jPrice" style="line-height:22px;"> 新车价:${detailsMap.value.newprice }万<br/>
          <span>为您节省:${detailsMap.value.newprice-detailsMap.value.CPrice }万</span><br/>
      ...
      <c:if test="${detailsMap.value.CState=='在售' }">
      <div class="byBtn clearfix"> <c:if test="${!empty userinfo }"><a  onclick="sendDg(${detailsMap.value.CId },${detailsMap.value.UId },'needdg')" >立即抢定</a> </c:if><a href="CarInfo.action?op=showCompare&uid=${detailsMap.value.UId }&cid=${detailsMap.value.CId }" class="balance_2">加入对比</a>
        <div class="jiathis_style_32x32"> <c:if test="${!empty userinfo }"><a onclick="sendSc(${detailsMap.value.CId },${detailsMap.value.UId })"  class="jiathis " style="text-decoration:none; text-align:center;display:inline-block;height:35px;  margin-right:12px">收藏</a> </c:if></div>
        <!--<a href="#">置换</a>--> 
