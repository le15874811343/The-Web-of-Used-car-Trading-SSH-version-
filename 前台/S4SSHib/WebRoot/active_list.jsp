    ...
    <div class="a-wb">
      <div class="wrap">
        ...
        <div class="a-div">
          ...
          <li class="clearfix"> 
              <a href="Trends_showTeia.action?tr_id=${trends.value.trId }" target="_blank">
              <div class="img left dInline"> <img width="338" height="221" src="${ trends.value.trImg }"  /> </div>
              <div class="con right dInline">
                <div class="ne_tit clearfix">
                  <h2 class="left dInline"> ${ trends.value.trTitle } </h2>
                  <span class="right"> ${ trends.value.trDate } </span> </div>
                <p>${ trends.value.trText }</p>
           ...
           <div class="pages"> <c:if test="${curPage > 1}">
					<a href="Trends_showActive.action?jumpPage=1&tname=${tname}&bname=${bname }&cser=${cser}&price=${price }&distance=${distance}&age=${age }&emsi=${emsi}&order=${order}">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="Trends_showActive.action?jumpPage=${curPage - 1}&tname=${tname}&bname=${bname }&cser=${cser}&price=${price }&distance=${distance}&age=${age }&emsi=${emsi}&order=${order}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="Trends_showActive.action?jumpPage=${curPage + 1}&tname=${tname}&bname=${bname }&cser=${cser}&price=${price }&distance=${distance}&age=${age }&emsi=${emsi}&order=${order}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="Trends_showActive.action?jumpPage=${maxPage}&tname=${tname}&bname=${bname }&cser=${cser}&price=${price }&distance=${distance}&age=${age }&emsi=${emsi}&order=${order}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
