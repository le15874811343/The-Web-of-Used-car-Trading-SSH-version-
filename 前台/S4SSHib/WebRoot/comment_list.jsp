...
<div id="about">
  <div class="mTags wrap">
  	<a href="CarInfo.action">亿金名车广场</a>><a >客户留言</a>
   </div>
  <div class="wAbout">
    <div class="a-wa">
      <div class="wrap a-nav">
        <ul class="clearfix">
          ...
          <li><a class="on" href="Trends_showCom.action">客户留言</a></li>
    ...
    <div class="a-wb">
      <div class="wrap">
        ...
        <div class="a-div">
          ...
          <li class="clearfix"> 
              <a href="Trends_showComd.action?cid=${trends.value.CId }" target="_blank">
              <div class="img left dInline"> <img width="338" height="221" src="${ trends.value.CImg }"  /> </div>
              <div class="con right dInline">
                <div class="ne_tit clearfix">
                  <h2 class="left dInline"> ${ trends.value.CText } </h2>
                  <span class="right"> ${ trends.value.CDate } </span> </div>
                <p>${trends.value.CText }</p>
           ...
           <div class="pages"> <c:if test="${curPage > 1}">
					<a href="Trends_showCom.action?jumpPage=1&tname=${tname}&bname=${bname }&cser=${cser}&price=${price }&distance=${distance}&age=${age }&emsi=${emsi}&order=${order}">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="Trends_showCom.action?jumpPage=${curPage - 1}&tname=${tname}&bname=${bname }&cser=${cser}&price=${price }&distance=${distance}&age=${age }&emsi=${emsi}&order=${order}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="Trends_showCom.action?jumpPage=${curPage + 1}&tname=${tname}&bname=${bname }&cser=${cser}&price=${price }&distance=${distance}&age=${age }&emsi=${emsi}&order=${order}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="Trends_showCom.action?jumpPage=${maxPage}&tname=${tname}&bname=${bname }&cser=${cser}&price=${price }&distance=${distance}&age=${age }&emsi=${emsi}&order=${order}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
