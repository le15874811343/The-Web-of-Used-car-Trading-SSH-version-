...
{
alert("删除成功")
window.location.href="<%=basePath%>Manager_showyclsrdz.action"
}
else
{
alert("删除失败")}
	...
	<div class="main">
		...
		<c:if test="${!empty szCar }">
                            ...
                            <tr><td colspan="2" align="center">需求详述：</td><td colspan="5">${szCar.value.PMiaoshu }</td></tr>
                             ...
                             <div class="hPages">操作: <a onclick="del(${szCar.value.PId})">删除</a>   </div>
                            <div class="hPages">    </div>
                             <div class="hPages">    </div>
							</c:forEach>
							</c:if>
							 <div class="pages"> <c:if test="${curPage > 1}">
					<a href="/S4SSHManager/Manager_showyclsrdz.action?jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="/S4SSHManager/Manager_showyclsrdz.action?jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="/S4SSHManager/Manager_showyclsrdz.action?jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4SSHManager/Manager_showyclsrdz.action?jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
