...
<div id="header" class="wrap">
	...
	<div class="navbar">
		<ul class="clearfix">
			...
			<li class="current"><a href="/S4SSHManager/Trends_showcom.action">留言</a></li>
...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>修改评论</h2>
		<div class="manage">
			<form action="/S4SSHManager/Trends_tjupcom.action" method="post" onsubmit="return cheksub('maxAge','chekAge','zm','chekZm')">
				<input type="hidden" name="op" value="tjupcom"/>
				<input type="hidden" name="cid" value="${ucomment.CId }"/>
								<input type="hidden" name="qtp" value="${ucomment.CImg }"/>
				
				<table class="form">
				
				<tr><td>评论标题</td><td><input type="text" id="maxAge" name="maxAge" value="${ucomment.CBt }"  onblur="cheknull('maxAge','chekAge')"/><div id="chekAge" >${cmea }</div></td></tr>
				<tr><td>评论内容</td><td><textarea id="zm" readonly="readonly" name="zm"  onblur="cheknull('zm','chekZm')">${ucomment.CText }</textarea><div id="chekZm" style="display: none;"></div></td></tr>
				
				<tr><td>评论头图片</td><td ><div align="center" style="height:200;border:5px solid red; " id="fa1"><img src="/S4SSHManager/${ucomment.CImg }" /></div><div style=" height:0px; overflow:hidden; position:absolute;" ><input type="file" id="filePath" name="filePath" onchange="fileupload('filePath','fa1','ttp')"/></div>
           ...
           <input type="button" onclick="document.getElementById('filePath').click();" value="上传" /></td></tr>
				...
				<c:if test="${ucomment.CAdmin=='首页展示' }">
				<option selected="selected"  value="首页展示">首页展示</option>
				<option value="普通">普通</option>
				</c:if>
				<c:if test="${ucomment.CAdmin=='普通' }">
