...
var url="<%=basePath%>Trends_delcom.action?cid="+a;
...
{
alert("删除成功")
window.location.href="<%=basePath%>Trends_showcom.action"
}
else
{
alert("删除失败")}
		...
		<div style="width:750px; height:600px; overflow:scroll;">
			<table class="list">
				...
				<tr>
					...
					<td>${trends.value.CBt}</td>
					<td><img  width="200" src="${trends.value.CImg}"/></td>
					<td>${trends.value.CText}</td>
					<td>${trends.value.CAdmin}</td>
					<td>${trends.value.CDate}</td>
					<td class="w1 c"><a href="Trends_upcom.action?cid=${trends.value.CId }">修改</a> <a onclick=" udelete(${trends.value.CId})">删除</a></td>
				</tr>
			 ...
			 <div class="pages"> <c:if test="${curPage > 1}">
					<a href="Trends_showcom.action?jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="Trends_showcom.action?jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="Trends_showcom.action?jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="Trends_showcom.action?jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
