...
var url="<%=basePath%>Mgord_qzzz.action?cid="+cid+"&cuid="+cuid+"&uid="+uid;
jQuery.post(url,null,callback)
function callback(data){
if(data==1)
{
		...
		<!--<form method="get">
			...
			<table class="list">
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="/S4SSHManager/Mgord_showjyz.action?jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="/S4SSHManager/Mgord_showjyz.action?jumpPage=${curPage - 1}&">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="/S4SSHManager/Mgord_showjyz.action?jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4SSHManager/Mgord_showjyz.action?jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
