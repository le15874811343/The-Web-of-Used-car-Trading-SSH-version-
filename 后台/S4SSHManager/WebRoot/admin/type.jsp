...
var url="<%=basePath%>Model_deltype.action?tid="+a;
...
{
alert("删除成功")
window.location.href="<%=basePath%>Model_showtype.action"
}
else
{
alert("删除失败")}
	...
	<div class="main">
		...
		<div class="manage">
			<table class="list">
				...
				<tr>
					<td class="first w4 c">${brandMap.value.TId}</td>
					<td><img src="${brandMap.value.TClass }"/>${brandMap.value.TName }</td>
					<td>${brandMap.value.TCount }</td>
					<td class="w1 c"><a href="Model_uptype.action?tid=${brandMap.value.TId }">修改</a> <a onclick="udelete(${brandMap.value.TId})">删除</a></td>
				</tr>
				</c:forEach>
				</c:if>
