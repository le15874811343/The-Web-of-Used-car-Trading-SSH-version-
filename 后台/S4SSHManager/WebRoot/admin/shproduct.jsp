...
var url="<%=basePath%>MgCar_jjtg.action?cid="+a+"&uid="+b;
...
{
alert("操作成功")
window.location.href="<%=basePath%>MgCar_showwtg.action"
}
else
{
alert("删除失败")}
...
var url="<%=basePath%>MgCar_yxtg.action?cid="+a+"&uid="+b+"&newprice="+newprice;
jQuery.post(url,null,callback)
function callback(data)
{
if(data==1)
...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>审核中的商品</h2>
		<div class="manage">
		<form action="/S4SSHManager/MgCar_showshwhere.action" method="post">
		 ...
		 <select  name="sel2" id="sel2" onchange="getcx(this)" >
			...
			<table class="list">
				...
				<td>${carMap.UId }</td>
					...
					<td class="thumb" ><a target="_blank" href="/S4SSHib/CarInfo_showshdea.action?cid=${carMap.CId }&uid=${carMap.UId}"><img src="/S4SSHib/${carMap.CImg}" /></a><a target="_blank" href="/S4SSHib/CarInfo_showshdea.action?cid=${carMap.CId }&uid=${carMap.UId}">${carMap.CBrand } ${carMap.CSeries } ${carMap.CId }${carMap.CType }${carMap.CModel }&nbsp${carMap.CPrice }万元</a></td>
					<td><input type="text" name="newprice" id="newprice${carMap.CId }" onblur="chekprice('newprice${carMap.CId }','d${carMap.CId }','p${carMap.CId }')"/><div id="d${carMap.CId }" style="display: none"></div><input type="hidden" id="p${carMap.CId}" value="${ carMap.CPrice}"/></td><td>${carMap.CState }</td>
					<td class="w1 c"> <input type="button" value="允许" onclick="yxtg(${carMap.CId},${carMap.UId },'newprice${carMap.CId }','d${carMap.CId }','p${carMap.CId }')"></input><input type="button" value="拒绝" onclick="jjtg(${carMap.CId},${carMap.UId })"></input></td>
				</tr>
				</c:forEach>
				</c:if>
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="/S4SSHManager/MgCar_showshwhere.action?cpp=${pp }&ccx=${cx}&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="/S4SSHManager/MgCar_showshwhere.action?cpp=${pp }&ccx=${cx}&jumpPage=${curPage - 1}&">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="/S4SSHManager/MgCar_showshwhere.action?cpp=${pp }&ccx=${cx}&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4SSHManager/MgCar_showshwhere.action?cpp=${pp }&ccx=${cx}&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
