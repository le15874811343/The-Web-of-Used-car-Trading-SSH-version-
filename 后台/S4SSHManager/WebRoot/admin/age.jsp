...
var url="<%=basePath%>Model_delage.action?aid="+a;
...
{
alert("删除成功")
window.location.href="<%=basePath%>Model_showage.action"
}
else
{
alert("删除失败")}
	...
	<div class="main">
		...
		<div class="manage">
			<table class="list">
				...
				<tr>
					<td class="first w4 c">${brandMap.value.AId}</td>
					<td>${brandMap.value.AName }</td>
					<td>${brandMap.value.ACount }</td>
					<td class="w1 c"><a href="Model_upage.action?aid=${brandMap.value.AId }">修改</a> <a onclick="udelete(${brandMap.value.AId})">删除</a></td>
				</tr>
				</c:forEach>
				</c:if>
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="Model_showage.action?jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="Model_showage.action?jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="Model_showage.action?jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="Model_showage.action?jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
