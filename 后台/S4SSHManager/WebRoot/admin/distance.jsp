...
var url="<%=basePath%>Model_deldis.action?did="+a;
...
{
alert("删除成功")
window.location.href="<%=basePath%>Model_showdistance.action"
}
else
{
alert("删除失败")}
	...
	<div class="main">
		...
		<div class="manage">
			<table class="list">
				...
				<tr>
					<td class="first w4 c">${brandMap.value.DId}</td>
					<td>${brandMap.value.DName }</td>
					<td>${brandMap.value.DCount }</td>
					<td class="w1 c"><a href="Model_updis.action?did=${brandMap.value.DId }">修改</a> <a onclick="udelete(${brandMap.value.DId})">删除</a></td>
				</tr>
				</c:forEach>
				</c:if>
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="Model_showdistance.action?jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="Model_showdistance.action?jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="Model_showdistance.action?jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="Model_showdistance.action?jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
