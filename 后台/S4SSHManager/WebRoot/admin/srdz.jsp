...
<div id="main" class="wrap">
	...
	<div class="main">
		...
		<c:if test="${!empty szCar }">
                            ...
                            <c:if test="${szCar.value.PState=='处理中' }">
                            <div class="hPages">操作: <input type="button" onclick="ycl(${szCar.value.PId})" value="已处理"></input> <a onclick="del(${szCar.value.PId})">删除</a>   </div></c:if>
                             <c:if test="${szCar.value.PState=='已处理' }">
                             <div class="hPages">操作: <a onclick="del(${szCar.value.PId})">删除</a>   </div></c:if>
                            <div class="hPages">    </div>
                             <div class="hPages">    </div>
							</c:forEach>
							</c:if>
							 <div class="pages"> <c:if test="${curPage > 1}">
					<a href="/S4SSHManager/Manager_showsrdz.action?jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="/S4SSHManager/Manager_showsrdz.action?jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="/S4SSHManager/Manager_showsrdz.action?jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4SSHManager/Manager_showsrdz.action?jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
