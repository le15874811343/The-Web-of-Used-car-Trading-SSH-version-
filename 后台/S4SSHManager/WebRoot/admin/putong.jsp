...
{
alert("删除成功")
window.location.href="<%=basePath%>Manager_showuser.action"}
else
{
alert("删除失败")}
}
		...
		<div style="width:750px; height:600px; overflow:scroll;">
			<table class="list"  >
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="Manager_showuser.action?jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="Manager_showuser.action?jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="Manager_showuser.action?jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="Manager_showuser.action?jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
