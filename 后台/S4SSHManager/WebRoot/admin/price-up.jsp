...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>修改车价区间</h2>
		<div class="manage">
			<form action="/S4SSHManager/Model_tjupprice.action" method="post" onsubmit="return chesub('minAge','maxAge','万','cAge','chekAge')">
				<input type="hidden" name="op" value="tjupprice"/>
				<input type="hidden" name="pid" value="${price.PId }"/>
				<table class="form">
				<tr><td>目前车价区间</td><td><input type="text"  value="${price.PName }" disabled="disabled"/></td></tr>
				...
				<tr><td>热度</td><td colspan="4"><input type="text" id="count" value="${price.PCount }" name="count" onblur="chekCount()" /><div id="d2" >${mea }</div></td><td></td></tr>
				<tr><td>注</td><td>只输入最小车价则记录为X万以上，只输入最大车价则记录为X万以内,两者都输则记录为X-X万,热度高的会在前台优先展示,不输入则记为0</td></tr>	
					
					<tr>
						<td><input type="hidden" id="cAge" value="${price.PName }" name="cAge"/></td>
