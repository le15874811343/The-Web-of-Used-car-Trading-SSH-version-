...
{
alert("删除成功")
window.location.href="<%=basePath%>Trends_showactive.action"
}
else
{
alert("删除失败")}
		...
		<div style="width:750px; height:600px; overflow:scroll;">
			<table class="list">
				...
				<tr>
					...
					<td class="w1 c"><a href="Trends_upactive.action?tid=${trends.value.trId }">修改</a> <a onclick="udelete(${trends.value.trId})">删除</a></td>
				</tr>
			 ...
			 <div class="pages"> <c:if test="${curPage > 1}">
					<a href="Trends_showactive.action?jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="Trends_showactive.action?jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="Trends_showactive.action?jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="Trends_showactive.action?jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
