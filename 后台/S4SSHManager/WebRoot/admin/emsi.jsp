...
var url="<%=basePath%>Model_delemsi.action?eid="+a;
...
{
alert("删除成功")
window.location.href="<%=basePath%>Model_showemsi.action"
}
else
{
alert("删除失败")}
	...
	<div class="main">
		...
		<div class="manage">
			<table class="list">
				...
				<tr>
					<td class="first w4 c">${brandMap.value.EId}</td>
					<td>${brandMap.value.EName }</td>
					<td>${brandMap.value.ECount }</td>
					<td class="w1 c"><a href="Model_upemsi.action?eid=${brandMap.value.EId }">修改</a> <a onclick="udelete(${brandMap.value.EId})">删除</a></td>
				</tr>
				</c:forEach>
				</c:if>
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="Model_showemsi.action?jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="Model_showemsi.action?jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="Model_showemsi.action?jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="Model_showemsi.action?jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
