...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>修改车龄</h2>
		<div class="manage">
			<form action="/S4SSHManager/Model_tjupage.action" method="post" onsubmit="return chesub('minAge','maxAge','年','cAge','chekAge')">
				<input type="hidden" name="op" value="tjupage"/>
				<input type="hidden" name="aid" value="${age.AId}"/>
				<table class="form">
				<tr><td>目前车龄区间</td><td><input type="text" value="${age.AName }" disabled="disabled"/></td></tr>
				...
				<tr><td>热度</td><td colspan="4"><input type="text" id="count" name="count" value="${age.ACount }" onblur="chekCount()" /><div id="d2" style="display: none;"></div></td><td></td></tr>
				<tr><td>注</td><td>只输入最小车龄则记录为X年以上，只输入最大车龄则记录为X年以内,两者都输则记录为X-X年,热度高的会在前台优先展示,不输入则记为0</td></tr>	
					
					<tr>
						<td><input type="hidden" id="cAge" value="${age.AName }" name="cAge"/></td>
