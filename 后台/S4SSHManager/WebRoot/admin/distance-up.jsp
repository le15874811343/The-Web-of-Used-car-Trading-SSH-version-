...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>距离区间更改</h2>
		<div class="manage">
			<form action="/S4SSHManager/Model_tjupdis.action" method="post" onsubmit=" return chesub('minAge','maxAge','万公里','cAge','chekAge')">
				<input type="hidden" name="op" value="tjupdis"/>
				<input type="hidden" name="did" value="${distance.DId }"/>
				<table class="form">
				<tr><td>目前距离区间</td><td><input type="text" value="${distance.DName }" disabled="disabled"/></td></tr>
				...
				<tr><td>热度</td><td colspan="4"><input type="text" value="${distance.DCount }" id="count" name="count" onblur="chekCount()" /><div id="d2" style="display: none;"></div></td><td></td></tr>
				<tr><td>注</td><td>只输入最小距离则记录为X万公里以上，只输入最大距离则记录为X万公里以内,两者都输则记录为X-X万公里,热度高的会在前台优先展示,不输入则记为0</td></tr>	
					
					<tr>
						<td><input type="hidden" id="cAge" value="${distance.DName }" name="cAge"/></td>
