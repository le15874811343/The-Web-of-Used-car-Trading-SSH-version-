...
{
alert("删除成功")
window.location.href="<%=basePath%>MgCar_showlist.action"
}
else
{
alert("删除失败")}
...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>所有商品</h2>
		<div class="manage">
		<form action="/S4SSHManager/MgCar_showwhere.action" method="post">
		 ...
		 <select  name="sel2" id="sel2" onchange="getcx(this)" >
			...
			<table class="list">
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="/S4SSHManager/MgCar_showwhere.action?cpp=${pp }&ccx=${cx}&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="/S4SSHManager/MgCar_showwhere.action?cpp=${pp }&ccx=${cx}&jumpPage=${curPage - 1}&">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="/S4SSHManager/MgCar_showwhere.action?cpp=${pp }&ccx=${cx}&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4SSHManager/MgCar_showwhere.action?cpp=${pp }&ccx=${cx}&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
