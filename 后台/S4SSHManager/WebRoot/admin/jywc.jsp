		...
		<!--<form method="get">
			...
			<table class="list">
				...
				<tr>
					...
					<c:if test="${car.value.CId==sm.value.CId }">
					<td class="w1 c">${car.value.CPrice }万元</td>
					<td class="w1 c">${car.value.CMcsj }</td>
					</c:if>
					</c:forEach>
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="/S4SSHManager/Mgord_showjywc.action?jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="/S4Manager/Mgord_showjywc.action?jumpPage=${curPage - 1}&">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="/S4SSHManager/Mgord_showjywc.action?jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4SSHManager/Mgord_showjywc.action?jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
