 ...
 */
	public Map<Long, Carinfo> getCarInfoByBrand(Carinfo carInfo) {
		...
		List<Carinfo> clist=super.getHibernateTemplate().find(hql,new Object[]{carInfo.getCBrand()}); //获取结果集
			//遍历结果集 加入Map中
		for(Carinfo c:clist){
			carInfoMap.put(c.getCId(), c);
		}
 ...
 */
	public Map<Long, Carinfo> getCarByWhere(Carinfo carInfo) {
		...
		List<Carinfo> clist=	super.getHibernateTemplate().find(sql.toString(),o); //获取结果集
		//遍历结果集 ，加入map中
		for(Carinfo c:clist){
			carInfoMap.put(c.getCId(), c);
		}
 ...
 */	
   ...
   */
	public int updateCarAll(Carinfo carInfo) {
		// TODO Auto-generated method stub
		StringBuffer sql=new StringBuffer("update Carinfo set CBrand=?,CSeries=?,CReleaseyear=?,CVolume=?,CGeartype=?,CCode=?,CModel=?,CLicencetime=to_date(?,'yyyy/mm'),CDistance=?,CEmissionstandard=?,CPrice=?,CImg=?,CType=?,CCzzx=? where CId=?");
		List<Object> parmas=new ArrayList<Object>();
		//参数准备
		parmas.add(carInfo.getCBrand());
   ...
   */
	public int deletecarinfouser(Carinfo u) {
		...
		String hql="from  Carinfo where UId=?";
		int count=0;
	List<Carinfo> imageslist=	super.getHibernateTemplate().find(hql,u.getUId()); //获取持久化对象
	try{
	for(Carinfo i:imageslist){
