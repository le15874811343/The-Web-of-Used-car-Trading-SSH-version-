...
public class HardwareConfigDaoImpl extends BaseDao implements IHardwareConfig {
	...
	List<Hardwareconfig> blist=	 super.getHibernateTemplate().find(hql, h.getUId());
		try{
	...
	public int deletecidhardwareconfiguser(Hardwareconfig h) {
		...
		int count=0;
		try{
			super.getHibernateTemplate().delete(h);
			count=1;
		}
		catch (Exception e) {
			// TODO: handle exception
	...
	public boolean checkcidhardwareconfig(Hardwareconfig h) {
		...
		boolean flag=false;
		List<Hardwareconfig> blist=	 super.getHibernateTemplate().find(hql, h.getCId());
		if(blist.size()>0){
			flag=true;
		}
		return flag;
